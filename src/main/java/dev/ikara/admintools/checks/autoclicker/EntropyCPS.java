...
public class EntropyCPS implements PacketCPS {
    ...
    public double finish() {
        ...
        for(long dt:intervals){ long ms=dt/1_000_000; freq.put(ms,freq.getOrDefault(ms,0)+1); }
        double ent=0, n=intervals.size(); double log2=log(2);
        for(int c:freq.values()){ double p=c/n; ent-=p*(log(p)/log2); }
        double maxEnt=log(freq.size())/log2;
        return max(0,(1.0-min(1.0,ent/maxEnt))*100);
    }
    @Override public String hoverText(){ return String.format("Entropy ratio: %.2f", finish()/100); }
